/* --------------------------------------------------------------------------
   AdminReports.jsx – vivid-blue Admin UI (overall system reports)
   • Uses shared <AdminTopNavBar /> component for the fixed top bar
   • Dynamic left-padding (88 px ⇆ 256 px) in sync with sidebar collapse
...
export default function AdminReports() {
  ...
  useEffect(() => {
    const unsub = onAuthStateChanged(auth, (u) => {
      if (u) {
        setAdminName(u.displayName || u.email?.split("@")[0] || "Administrator");
      } else {
        navigate("/login");
      }
    });
  ...
  /* ────────────────────────────────────────
     Data fetch – ALL data across system
  ──────────────────────────────────────── */
  useEffect(() => {
    const fetchData = async () => {
      ...
      for (const r of resultsSS.docs) {
        ...
        attemptsRowsTmp.push({
          id         : r.id,
          name       : rowName,
          quizTitle  : quizTitlesMap[res.quizId] || res.quizId,
          score      : res.score,
          total      : res.total,
          percent,
  ...
  const exportPDF = () => {
    ...
    doc.setFontSize(16);
    doc.text("Quiz Results Report (All Teachers)", 14, 16);
    const tableHeaders = [
      "Student",
      "Quiz",
      "Score",
